...
export default function FloatingFooter() {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            const atBottom = currentScrollY + windowHeight >= docHeight - 100;
            setIsAtBottom(atBottom);

            // Hide when scrolling down rapidly (but not at bottom)
            if (!atBottom && currentScrollY > lastScrollY && currentScrollY > 400) {
                setIsVisible(false);
            } else {
                setIsVisible(true);
            }
    ...
    const links = [
        ...
        { icon: Shield, label: t("privacy_policy") || "Gizlilik Politikası", href: "/privacy-policy", color: "text-rose-500" },
        { icon: ScrollText, label: t("disclosure_text") || "Aydınlatma Metni", href: "/disclosure", color: "text-cyan-500" },
    ...
    // ===== AT BOTTOM: collapsed FAB in bottom-right =====
    if (isAtBottom) {
        return (
            <div className={`fixed bottom-6 right-6 z-50 transition-all duration-700 ease-in-out ${isVisible ? "translate-y-0 opacity-100 scale-100" : "translate-y-20 opacity-0 scale-75"}`}>
                {/* Expanded mini-menu */}
                {isExpanded && (
                    <div className="absolute bottom-16 right-0 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-700/80 rounded-2xl shadow-2xl shadow-black/10 dark:shadow-black/40 p-2 mb-2 animate-in slide-in-from-bottom-4 fade-in duration-300">
                ...
                <button
                    ...
                    className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 shadow-lg shadow-black/15 dark:shadow-black/40 ${isExpanded
    ...
    // ===== NOT AT BOTTOM: centered bar =====
    return (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-700 ease-in-out ${isVisible ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"}`}>
